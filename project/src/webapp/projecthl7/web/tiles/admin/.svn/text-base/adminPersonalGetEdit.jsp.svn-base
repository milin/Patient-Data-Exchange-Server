...
<input type="hidden" name="headName" value="${requestScope.personal.title} ${requestScope.personal.name} ${requestScope.lastName} ${param.headName}"/>
		<a href="listAdminPersonal.do?id=${requestScope.personal.id}&sidebar=3"><bean:message key="applicationForm.hospital.listPersonal.header"/></a> 
...
<h2><bean:message key="applicationForm.hospital.Personal.editHeader"/></h2> 
   		 <form action="editAdminPersonal.do" method="post">
   		 <input type="hidden" name="personalId" value="${requestScope.personal.id}${param.personalId}">
   		        <fieldset>	
