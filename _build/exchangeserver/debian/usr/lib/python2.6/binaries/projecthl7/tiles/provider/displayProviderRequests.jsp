...
<h2><bean:message key="applicationForm.hospital.providerRequests" /></h2>
...
<table border="1" width="90%" cellpadding="1" cellspacing="1">
	<tr>
		...
		<th><bean:message
			key="applicationForm.hospital.addPersonal.prority" /></th>
	</tr>
	<c:choose>
			...
			<c:forEach var="provider" items="${requestScope.provider}">
				...
				<tr bgcolor="#FFFFCC">
					...
					<tr>
						...
						<td align="center">${provider.provider.name}</td>
						<td align="center"><a
							href="addProviderRequest.do?id=${provider.id}"><bean:message
							key="applicationForm.hospital.providerRequests" /></a></td>
						<td align="center"><c:choose>
							<c:when
								test="${provider.provider.statusOnline==0 or provider.provider.statusOnline==null}">
														Offine
													</c:when>
							<c:otherwise> 
														Online
		...
		<td align="center" colspan="4">
			...
			<%
							long index = (Long) request.getAttribute("index");
								if (index > 1) {
							%>
			<a href="displayProviderRequests.do?index=1"><<</a>&nbsp;
							<a href="displayProviderRequests.do?index=<%=(index - 1)%>"><</a>&nbsp;
							<% } %>
			<%
								long totalPages = (Long) request.getAttribute("totalPages");
									for (long i = 1; i <= totalPages; i++) {
							%>
			<a href="displayProviderRequests.do?index=<%=i%>"> <%
								if(i==index){
								out.print("<b>"+i+"</b>");
								} else {
									out.print(i);
			...
			<a href="displayProviderRequests.do?index=<%=(index + 1)%>">></a>&nbsp;
								<a href="displayProviderRequests.do?index=<%=totalPages%>">>></a>
			<% }%>
			</c:if>
		</td>
	</tr>
